...
import $ from "jquery";
import { parseCabinetColumn, parseAuthor, parseMasterData, parseCabinetTopList, parseCabinetRankList } from "@/utils/html-parser";

/**
 * 获取侧边栏的部分随笔分类列表。直接调用解析函数解析 home 节点下的侧边栏。
 */
export function getCabinetColumn() {
  return parseCabinetColumn($("#home").find("#blog-sidecolumn").html());
}
...
export function getAuthor() {
  return parseAuthor($("#home").find("#blog-news").html());
}
...
export function getMasterData() {
  return parseMasterData($("#home").find(".blogStats").html());
}
...
export function getCabinetTopList() {
  return parseCabinetTopList($("#home").find("#sidebar_topviewedposts").html());
}
...
export function getCabinetRankList() {
  return parseCabinetRankList($("#home").find("#sidebar_scorerank").html());
}
