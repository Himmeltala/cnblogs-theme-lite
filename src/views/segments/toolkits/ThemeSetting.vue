...
watch(localSetting, newVal => {
  LiteConfig.eleHtml.style.setProperty(LiteCssVars.themeColor, newVal.theme.color);
  LiteConfig.eleHtml.style.setProperty(LiteCssVars.bgFilter, newVal.background.filter + "px");
  LiteConfig.eleHtml.style.setProperty(LiteCssVars.mainFont, newVal.theme.mainFont);
  LiteConfig.eleHtml.style.setProperty(LiteCssVars.codeFont, newVal.theme.codeFont);
});
</script>
...
<template>
  <div class="l-size-2">
    <div class="mb-4">
      ...
      <el-color-picker size="small" :predefine="colors" show-alpha v-model="localSetting.theme.color" />
    </div>
    <div class="mb-4">
    ...
    <div class="mb-4">
      ...
      <el-input size="small" v-model="localSetting.theme.mainFont" />
    </div>
    <div class="mb-4">
      <div class="mb-2">代码字体</div>
      <el-input size="small" v-model="localSetting.theme.codeFont" />
    </div>
    <el-collapse v-model="active" accordion>
      <el-collapse-item title="卡片样式设置" v-show="localSetting.card.open">
        <div class="mb-4">
          <div class="mb-2">设置卡片背景颜色</div>
          <el-color-picker size="small" :predefine="colors" show-alpha v-model="localSetting.card.color" />
        </div>
        <div class="mb-4">
      ...
      <el-collapse-item title="背景样式设置" v-show="localSetting.background.open">
        <div class="mb-4">
          ...
          <el-input size="small" v-model="localSetting.background.src" placeholder="请输入网络地址" />
        </div>
        <div>
          <div class="mb-2">背景毛玻璃效果</div>
          <el-slider v-model="localSetting.background.filter" />
        </div>
      </el-collapse-item>
    </el-collapse>
  </div>
