...
const left = computed(() => {
  return localSetting.value.cabinet.left.pin && localSetting.value.cabinet.position.break
    ? localSetting.value.cabinet.position.left + "vw"
    : 0;
});

const block = computed(() => {
  return !props.disabled && !localSetting.value.cabinet.left.pin;
});
...
<template>
  <ContextMenu
    id="l-lcabinet"
    class="fixed top-0 left-0 z-4"
    :style="{ left: left }"
    :class="{ 'show-lcabinet': block, 'hidden-lcabinet': hidden, 'fixed-lcabinet': fixed }"
    style="width: var(--l-cabinet-width)">
    <Card
      class="l-size-2"
      :class="{ 'l-box-bg px-2': !localSetting.card.open }"
      :padding="localSetting.cabinet.left.padding"
      :margin="localSetting.cabinet.left.margin">
          ...
          <el-tabs v-model="tabActive" stretch>
            <el-tab-pane label="博客常用项" name="first">
              <template v-if="columnList">
                ...
                <router-link :to="RouterConstants.Path.WORKS_BY_CALENDAR()">
                ...
                <el-collapse v-model="commonCollActive" accordion>
                  <el-collapse-item v-if="columnList.essaySort.length">
                    ...
                    <div
                      ...
                      <router-link :to="RouterConstants.Path.WORKS_BY_SORT(item.id)">
                        {{ item.text }}
                      </router-link>
                    </div>
                  </el-collapse-item>
                  <el-collapse-item v-if="columnList.essayArchive.length">
                    ...
                    <div
                      ...
                      <router-link :to="RouterConstants.Path.WORKS_BY_ARCHIVE('p', item.id)">
                        {{ item.text }}
                      </router-link>
                    </div>
                  </el-collapse-item>
                  ...
                  <el-collapse-item title="文章档案" v-if="columnList.articleArchive.length">
                    ...
                    <div
                      ...
                      <router-link :to="RouterConstants.Path.WORKS_BY_ARCHIVE('a', item.id)">
                        {{ item.text }}
                      </router-link>
                    </div>
                  </el-collapse-item>
                  <el-collapse-item title="标签分类" v-if="columnList.tagList.length">
                    ...
                    <div
                      ...
                      <router-link :to="RouterConstants.Path.WORKS_BY_MARK(item.id)">
                        {{ item.text }}
                      </router-link>
                    </div>
                    <div>
                      <router-link :to="RouterConstants.Path.MARK_LIST()">更多...</router-link>
                    </div>
                  </el-collapse-item>
                  <el-collapse-item title="最新随笔" v-if="columnList.latestEssayList.length">
                    ...
                    <div
                      ...
                      <router-link :to="RouterConstants.Path.WORKS(item.id)">
                        {{ item.text }}
                      </router-link>
                    </div>
                  </el-collapse-item>
    ...
    <template #content>
      <el-collapse v-model="settingCollActive" accordion>
        ...
        <el-collapse-item title="盒子模型">
          <div class="ml-4">
            <BoxSetting :padding="localSetting.cabinet.left.padding" :margin="localSetting.cabinet.left.margin" />
          </div>
        </el-collapse-item>
      </el-collapse>
    </template>
...
.fixed-lcabinet {
  left: calc(calc(var(--l-content-width) / 2) - calc(var(--l-cabinet-width) + 2rem)) !important;
}
...
@keyframes showlcabinet {
  @for $i from 0 to $quota {
    #{$i * 10%} {
      transform: translateX(calc(calc(-1 * var(--l-cabinet-width)) + calc($i * calc(var(--l-cabinet-width) / 10))));
    }
  }
}
...
.hidden-lcabinet {
  ...
  transform: translateX(calc(-1 * var(--l-cabinet-width)));
}

@keyframes hiddenlcabinet {
  @for $i from 0 to $quota {
    #{$i * 10%} {
      transform: translateX(calc($i * calc(calc(-1 * var(--l-cabinet-width)) / 10)));
    }
  }
}
</style>
